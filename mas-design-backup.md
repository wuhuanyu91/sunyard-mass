...
智能模型路由访问平台（Model Access & Scheduling，简称 MAS）是一个 **OpenAI 兼容的 HTTP 代理服务**，插在智能体与底层大模型推理引擎之间，所有大模型调用先经过它，完成缓存/路由/管控后再转发给真正的推理引擎。

**本质上就是：给所有大模型调用加了一个"总闸"，实现统一管控。**
...
| 典型（全链路）       | ~41ms | L1 + L2 + L3 + L4 + 转发 |
| 最差（含压缩 + 审核）  | ~60ms | 全链路 + 上下文压缩            |
...
**职责**：根据请求内容判断意图，选择最合适的模型和推理引擎。

```mermaid
flowchart TB
    INPUT(["请求进入 L3"]) --> CLASSIFY["意图分类<br/>轻量分类模型 0.5B<br/>~20ms"]
    CLASSIFY --> ROUTE{"路由决策"}
    ROUTE -->|"代码生成"| A["deepseek‑coder<br/>vLLM 集群"]
    ROUTE -->|"通用对话"| B["qwen‑72b<br/>vLLM 集群"]
    ROUTE -->|"知识问答/RAG"| C["qwen‑72b + RAG<br/>SGLang 集群"]
    ROUTE -->|"文本分析"| D["qwen‑7b<br/>Ollama 集群"]
    ROUTE -->|"多模态"| E["qwen‑vl<br/>vLLM 集群"]
    ROUTE -->|"Embedding"| F["bge‑m3<br/>专用 Embedding 服务"]
    ROUTE -->|"复杂推理"| G["deepseek‑r1<br/>vLLM 集群"]
```


#### 5.3.1 意图分类

使用轻量级分类模型（0.5B 参数），将请求分为以下意图类别：

| 意图类别         | 说明          | 默认路由模型         |
| ------------ | ----------- | -------------- |
| `chat`       | 通用对话        | qwen‑72b       |
| `code`       | 代码生成/补全     | deepseek‑coder |
| `rag`        | 知识问答（需检索增强） | qwen‑72b + RAG |
| `analysis`   | 文本分析/总结/提取  | qwen‑7b        |
| `multimodal` | 图片/多模态理解    | qwen‑vl        |
| `embedding`  | 文本向量化       | bge‑m3         |
| `reasoning`  | 复杂推理/数学     | deepseek‑r1    |
...
| 策略       | 说明                              |
| -------- | ------------------------------- |
| **负载均衡** | 同意图下多实例时，按权重 + 最少连接数分配          |
| **模型指定** | 请求中明确指定 `model` 字段时，跳过意图分类，直接路由 |
| **故障转移** | 目标引擎不可用时，自动切换到备用引擎              |
| **灰度路由** | 支持按用户/应用维度将流量路由到新版本模型           |

**降级策略**：意图分类模型不可用时，默认路由到通用对话模型（qwen‑72b）。
